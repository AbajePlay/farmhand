...
sealed class Loader(val id: String) {
	...
	/** The manifest filename can depend on the Minecraft version — see NeoForge. */
	open fun modManifestPath(ctx: Context): String = modManifestPath

	open fun excludedResources(ctx: Context): List<String> = excludedResources
	...
	object Fabric : Loader("fabric") {
		...
		override fun generateManifest(ctx: Context): String {
			val manifest = FabricManifest(
				...
				// Farmhand is server-side: it has neither a client entrypoint nor data generation.
				// Declaring classes that do not exist makes Fabric Loader crash on startup.
				entrypoints = mapOf(
	...
	object NeoForge : ForgeLike("neoforge") {
		...
		// NeoForge 20.4 reads META-INF/mods.toml; from 20.5 on it reads neoforge.mods.toml.
		// Verified against NeoForge's own manifests: 20.4.251 -> mods.toml,
		// 20.6.139 -> neoforge.mods.toml. With the wrong name the mod silently fails to load:
		// the server starts fine, there is simply no mod in it.
		private fun legacyManifest(ctx: Context) = ctx.stonecutter.eval(ctx.currentMcVersion, "<1.20.5")

		override fun modManifestPath(ctx: Context) =
			if (legacyManifest(ctx)) "META-INF/mods.toml" else modManifestPath

		override fun excludedResources(ctx: Context) =
			if (legacyManifest(ctx)) (super.excludedResources + "META-INF/neoforge.mods.toml") + "pack.mcmeta"
