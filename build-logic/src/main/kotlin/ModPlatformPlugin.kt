...
abstract class ModPlatformPlugin @Inject constructor() : Plugin<Project> {
	...
	private fun Project.configureJava(ctx: Context) {
		extensions.configure<JavaPluginExtension>("java") {
			// A mod has no use for a javadoc jar: nobody consumes it, it lengthens every build
			// and it fails on non-ASCII comments (Javadoc has its own encoding, not JavaCompile's).
			withSourcesJar()
			sourceCompatibility = ctx.javaVersion
			targetCompatibility = ctx.javaVersion
			// Every Minecraft version group needs its own JDK (1.20.x -> 17/21, 26.x -> 25).
			// Without a toolchain Gradle compiles with the daemon's own javac and fails with
			// "invalid source release". Missing JDKs are provisioned by foojay-resolver.
			toolchain.languageVersion.set(JavaLanguageVersion.of(ctx.javaVersion.majorVersion))
		}

		// Before Java 18 javac used the platform encoding and failed on non-ASCII sources.
		// Pin UTF-8 so builds do not depend on the machine's locale.
